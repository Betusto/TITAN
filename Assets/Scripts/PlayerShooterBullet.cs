...
public class PlayerShooterBullet : MonoBehaviour
{
    float speed;
    ...
    void OnTriggerEnter2D(Collider2D col)
    {
        ...
        {
            col.gameObject.GetComponent<EnemyControl>().Hit(3);
            //destruye la bala
            Destroy(gameObject);
        }
        ...
        {
            col.gameObject.GetComponent<Enemy2Control>().Hit(3);
            Destroy(gameObject);
        }

        if (col.tag == "EnemyShip3Tag")
        {
            col.gameObject.GetComponent<Enemy3Control>().Hit(3);
            Destroy(gameObject);
        }

        if (col.tag == "EnemyShip4Tag")
        {
            col.gameObject.GetComponent<Enemy4Control>().Hit(3);
            Destroy(gameObject);
        }

        if (col.tag == "EnemyShip5Tag")
        {
            col.gameObject.GetComponent<Enemy5Control>().Hit(3);
            Destroy(gameObject);
        }

        if (col.tag == "EnemyShip6Tag")
        {
            col.gameObject.GetComponent<Enemy6Control>().Hit(3);
            Destroy(gameObject);
        }
    }
}
